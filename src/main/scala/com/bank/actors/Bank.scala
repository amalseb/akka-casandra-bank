...
import com.bank.actors.PersistentBankAccount.Command.{CreateBankAccount, GetBankAccount}
import com.bank.actors.PersistentBankAccount.Response
import com.bank.actors.PersistentBankAccount.Response.{BankAccountCreatedResponse, GetBankAccountResponse}
    ...
    command match {
      ...
      case updateCmd @ UpdateBalance(id, _, _, replyTo) =>
        state.accounts.get(id) match {
          ...
          case None =>
            Effect.reply(replyTo)(BankAccountBalanceUpdateResponse(None)) //failed account search
        }
      case getCmd @ GetBankAccount(id, replyTo) =>
        state.accounts.get(id) match {
          ...
          case None =>
            Effect.reply(replyTo)(BankAccountBalanceUpdateResponse(None)) // failed search
        }
    }
    ...
    event match {
      case BankAccountCreated(id) =>
        val account = context.child(id) // exists after command handler,
          .getOrElse(context.spawn(PersistentBankAccount(id), id)) // does not exist in recovery mode, so needs to create one.
...
object BankPlayground {
  def main(args: Array[String]): Unit = {
    val rootBehaviour: Behavior[NotUsed] = Behaviors.setup {
      context =>
...
//        bank ! CreateBankAccount("Amal", "Euro", 10, responseHandler)
